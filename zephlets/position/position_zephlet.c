...
#include "position_zephlet_interface.h"

#include "position_zephlet.h"
#include <zephyr/kernel.h>
#include <zephyr/logging/log.h>

LOG_MODULE_DECLARE(position_zephlet, CONFIG_POSITION_ZEPHLET_LOG_LEVEL);

/* TODO: Add zephlet-specific resources (timers, work queues, threads) */
static int start(const struct zephlet *zephlet)
{
	struct position_zephlet_data *data = zephlet->data;
	...
	return position_zephlet_report_status(&status, K_MSEC(250));
}
...
static int config(const struct zephlet *zephlet, const struct msg_position_zephlet_config *config)
{
	struct position_zephlet_data *data = zephlet->data;
	...
	return position_zephlet_report_config(config, K_MSEC(250));
}

static int get_config(const struct zephlet *zephlet)
{
	struct position_zephlet_data *data = zephlet->data;
	struct msg_position_zephlet_config config;
...
static int get_events(const struct zephlet *zephlet)
{
	 ...
	 *       struct msg_position_zephlet_events event = {...};
	 *       position_zephlet_report_events(&event, K_NO_WAIT);
	 *   }
	 */
...
static struct position_zephlet_api api = {
	.start = start,
	.stop = stop,
...
static struct position_zephlet_data data = {
	.config = MSG_POSITION_ZEPHLET_CONFIG_INIT_ZERO,
	.status = {.is_running = false}
};

int position_zephlet_init_fn(const struct zephlet *self)
{
	int err = position_zephlet_set_implementation(self);
...
ZEPHLET_DEFINE(position_zephlet, position_zephlet_init_fn, &api, &data);