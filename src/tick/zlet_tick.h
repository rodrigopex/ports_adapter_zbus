 ...
 * only knows the command shape. Per-instance storage
 * (`struct tick_data`) and the init callback (`tick_init_fn`) are user
 ...
 * `ZEPHLET_NEW(tick, ...)` and passes its address as the
 * `_data` arg. Framework treats it as opaque; the tick handlers are
 * the only readers/writers.
 */
struct tick_data {
 ...
 *
 * Initialises the timer with `z` as user-data and marks the instance
 * ready. The configuration lives in `*z->config` (writable; the
 * `config` command mutates it in place). Pass as the `_init` arg of
 * ZEPHLET_NEW.
 */
