...
**One `prj.conf` setting is not optional:** `CONFIG_CBPRINTF_FULL_INTEGRAL=y`. Only fields wider than 32 bits need it — the printer uses width-correct conversions — and `typelab_config`'s `uint64`/`int64`/`fixed64`/`sfixed64` are exactly those, reporting `PB_TF_ERR_UNSUPPORTED` without it rather than printing a wrong number. A long message may also need `CONFIG_SHELL_CMD_BUFF_SIZE` and `CONFIG_SHELL_BACKEND_SERIAL_RX_RING_BUFFER_SIZE` raised above their defaults (256 and 64); the latter matters when a line is *pasted* rather than typed, which arrives faster than the shell drains it. Both are application choices rather than library requirements — the library never sees the shell — and the sizes above are for typelab's 18-field message, not a default to copy; the library's `docs/shell-integration.md` explains each. Enabling `CONFIG_ZEPHLETS_SHELL=y` needs nothing else — in particular `CONFIG_SHELL_WILDCARD` can stay at its default, since an RPC's raw argument is never tokenised and `*` or `?` inside a value survives intact.
