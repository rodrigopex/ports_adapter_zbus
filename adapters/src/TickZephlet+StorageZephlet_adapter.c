...
{
	const struct msg_tick_zephlet_report *report = zbus_chan_const_msg(chan);
	...
	case MSG_TICK_ZEPHLET_REPORT_STATUS_TAG:
		...
		break;
	case MSG_TICK_ZEPHLET_REPORT_CONFIG_TAG:
...
ZBUS_CHAN_ADD_OBS(chan_tick_zephlet_report, lis_tick_to_storage_adapter, 3);