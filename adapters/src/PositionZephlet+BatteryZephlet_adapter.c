...
{
	const struct msg_position_zephlet_report *report = zbus_chan_const_msg(chan);

	switch (report->which_position_report) {
	case MSG_POSITION_ZEPHLET_REPORT_EVENTS_TAG:
...
ZBUS_CHAN_ADD_OBS(chan_position_zephlet_report, lis_position_to_battery_adapter, 3);